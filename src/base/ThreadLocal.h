...
/**
 * $Id: ThreadLocal.h,v 1.1 2008/05/31 15:47:29 iamcamiel Exp $
 *
...
class CTLSAbstractSlot
	/// This is the base class for all objects
	/// that the ThreadLocalStorage class manages.
{
public:
	CTLSAbstractSlot();
	virtual ~CTLSAbstractSlot();
};


template <class C>
class CTLSSlot: public CTLSAbstractSlot
...
private:
	CTLSSlot(const CTLSSlot&);
	CTLSSlot& operator = (const CTLSSlot&);
...
public:
	...
	CTLSAbstractSlot*& get(const void* key);
		/// Returns the slot for the given key.
		
	static CThreadLocalStorage& current();
...
private:
	typedef std::map<const void*, CTLSAbstractSlot*> CTLSMap;
...
public:
	...
	{
		CTLSAbstractSlot*& p = CThreadLocalStorage::current().get(this);
		if (!p) p = new CSlot;
		return static_cast<CSlot*>(p)->value();
	}
	
private:
	CThreadLocal(const CThreadLocal&);
	CThreadLocal& operator = (const CThreadLocal&);
};
